...
from ocr_tesseract import image_to_bboxes, image_to_text
...
class TestOCRModule(unittest.TestCase):
    ...
    def update_api_specs(self, endpoint, request, response):
        ...
        if endpoint not in self.api_specs:
            self.api_specs[endpoint] = []
    ...
    def get_text_invalid_base64_image(self):
        ...
        self.assertIn("Invalid base64", response_data['error']['message'])  
    ...
    def get_bboxes_invalid_type(self):
        ...
        self.assertEqual(response_data['success'], False)
        self.assertEqual(response_data['error']['message'], "Invalid bbox_type: foo")
        request_obj = {
            'headers': {
                'Content-Type': "application/json",
            },
